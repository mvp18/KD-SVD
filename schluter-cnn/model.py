...
def Schluter_CNN(dropout_rate):
    ...
    model = Sequential()
    model.add(Conv2D(64, (3, 3), name='conv1', padding='valid', kernel_initializer='he_normal', input_shape=input_shape))
...
def CNN_small(dropout_rate):
    ...
    input_shape = (80, 115, 1)
    model = Sequential()
    model.add(Conv2D(32, (3, 3), name='conv1', padding='valid', kernel_initializer='he_normal', input_shape=input_shape))
    model.add(LeakyReLU(0.01))
    model.add(Conv2D(16, (3, 3), name='conv2', padding='valid', kernel_initializer='he_normal'))
    model.add(LeakyReLU(0.01))
    model.add(MaxPooling2D((3, 3), strides=(3, 3)))

    model.add(Conv2D(64, (3, 3), name='conv3', padding='valid', kernel_initializer='he_normal'))
    model.add(LeakyReLU(0.01))
    model.add(Conv2D(32, (3, 3), name='conv4', padding='valid', kernel_initializer='he_normal'))
    model.add(LeakyReLU(0.01))
    model.add(MaxPooling2D((3, 3), strides=(3, 3)))
